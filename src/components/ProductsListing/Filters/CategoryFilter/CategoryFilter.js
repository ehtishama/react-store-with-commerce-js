...
const CategoryFilter = () => {
  ...
  const handleCategorySelect = (e, selected) => {
    ...
    if (selected) selectedCategories.push(categoryId)
    else selectedCategories = selectedCategories.filter((category) => category !== categoryId)
    ...
    const result = products.filter((product) => {
      ...
      return selectedCategories.some((sC) => productCategories.includes(sC))
